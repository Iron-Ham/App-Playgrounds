...
public final class Vehicle: Model, @unchecked Sendable {
  public static let schema = "vehicles"
  ...
  @Siblings(through: FilmVehiclePivot.self, from: \.$vehicle, to: \.$film)
  public var films: [Film]

  @Siblings(through: PersonVehiclePivot.self, from: \.$vehicle, to: \.$person)
  public var pilots: [Person]

  public var url: URL {
    get {
      guard let id else {
        fatalError("Attempted to access Vehicle.url before the record had an assigned URL")
      }
      return id
    }
    set { id = newValue }
  ...
  public init() {
    ...
    self.cargoCapacity = ""
    self.consumables = ""
    self.vehicleClassRaw = ""
    self.created = .now
    self.edited = .now
  }

  public convenience init(from response: VehicleResponse) {
    self.init(
      url: response.url,
      name: response.name,
