...
struct FilmDetailModelTests {
  ...
  func updateSelectedFilmLoadsSummary() async throws {
    ...
    let summary = FluentPersistenceService.FilmRelationshipSummary(
      characterCount: 5,
      planetCount: 1,
      speciesCount: 0,
      starshipCount: 2,
  ...
  func expandingRelationshipFetchesEntities() async throws {
    ...
    let summary = FluentPersistenceService.FilmRelationshipSummary(characterCount: 1)
...
private actor FilmsServiceStorage {
  ...
  func makeService() -> FluentPersistenceService {
  ...
  private func makeChangeStream() async -> AsyncStream<FluentPersistenceService.ChangeBatch> {
  ...
  private func registerContinuation(
    _ continuation: AsyncStream<FluentPersistenceService.ChangeBatch>.Continuation
  ) async {
    await Task.yield()
...
private enum TestData {
  static func film(id: Int, title: String) -> FluentPersistenceService.FilmDetails {
...
private func loadedEntities(
  ...
  for relationship: FluentPersistenceService.Relationship
) -> [FluentPersistenceService.RelationshipEntity]? {
