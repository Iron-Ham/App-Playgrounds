...
struct FilmResponseTests {
  ...
  func initialization() throws {
    ...
    #expect(aNewHope.planets.count == 3)
    #expect(aNewHope.starships.count == 8)
    #expect(aNewHope.vehicles.count == 4)
    #expect(aNewHope.species.count == 5)
    #expect(aNewHope.characters.first?.absoluteString == "https://swapi.info/api/people/1")
    #expect(aNewHope.url.absoluteString == "https://swapi.info/api/films/1")
    #expect(aNewHope.openingCrawl.hasPrefix("It is a period of civil war."))
    #expect(aNewHope.openingCrawl.contains("DEATH"))
    #expect(
      aNewHope.release == Calendar.current
        .date(from: DateComponents(year: 1977, month: 5, day: 25))!
    )
    // Validate created / edited timestamps using ISO8601DateFormatter (drop fractional seconds entirely).
    // Decoder already discarded fractional precision, so compare second-level precision strings.
    let isoNoFrac = ISO8601DateFormatter()
    isoNoFrac.formatOptions = [.withInternetDateTime]
    #expect(isoNoFrac.string(from: aNewHope.created) == "2014-12-10T14:23:31Z")
    #expect(isoNoFrac.string(from: aNewHope.edited) == "2014-12-20T19:49:45Z")
  }
}

private let aNewHopeResponse = #"""
  {
    "title": "A New Hope",
    "episode_id": 4,
