...
struct DataStoreTests {
	...
	func importSnapshot_populatesEntitiesAndRelationships() throws {
		...
		try store.database.read { db in
			let filmRow = try Row.fetchOne(
				db,
				sql: "SELECT title, episodeId, director, producerRaw, releaseDate FROM films WHERE url = ?",
			...
			#expect(filmRow?["producerRaw"] == "Gary Kurtz, Rick McCallum")
	...
	func importSnapshot_clearsExistingDataBeforeReimport() throws {
		...
		let firstFilm = try FilmResponse(data: makeJSON(baseFilmPayload.merging([
			"title": "Film One",
			"episode_id": 4,
			"url": firstFilmURL.absoluteString,
		]) { _, new in new }))

		let secondFilm = try FilmResponse(data: makeJSON(baseFilmPayload.merging([
			"title": "Film Two",
			"episode_id": 5,
