...
struct DefaultMarkdownStyler: MarkdownStyler {
  ...
  private func applyStrong(
    using regex: NSRegularExpression, tokenLength: Int, to storage: MarkdownTextStorage
  ) {
    storage.enumerateMatches(
      for: regex, contentGroup: 1, leadingTokenLength: tokenLength, trailingTokenLength: tokenLength
    ) { match in
  ...
  private func applyLinks(using regex: NSRegularExpression, to storage: MarkdownTextStorage) {
    ...
    let matches = regex.matches(
      in: storage.string, range: NSRange(location: 0, length: nsString.length))
    for match in matches {
      ...
      let rightParenthesisRange = NSRange(
        location: fullRange.location + fullRange.length - 1, length: 1)

      for markerRange in [
        leftBracketRange, rightBracketRange, leftParenthesisRange, rightParenthesisRange,
      ] where markerRange.isValid(within: nsString.length) {
        storage.addAttributes(
          [
            .foregroundColor: colors.token
          ], range: markerRange)
  ...
  private func applyList(
    using regex: NSRegularExpression, markerGroup: Int, contentGroup: Int,
    to storage: MarkdownTextStorage
  ) {
    let nsString = storage.string as NSString
    let matches = regex.matches(
      in: storage.string, range: NSRange(location: 0, length: nsString.length))
    for match in matches {
      ...
      let paragraphStyle = listParagraphStyle(
        mergingWith: paragraphStyle(from: storage, at: lineRange.location))
      storage.addAttributes(
        [
  ...
  private func highlightTableSeparatorCharacters(
    in line: MarkdownTableLine, storage: MarkdownTextStorage
  ) {
    guard line.range.length > 0 else { return }
    let nsLine = line.text as NSString
    let length = nsLine.length
        ...
        if segmentLength > 0 {
          ...
          if trimmedRange.length > 0 {
            let globalRange = NSRange(
              location: line.range.location + trimmedRange.location, length: trimmedRange.length)
            storage.addAttributes(
              [
                .font: fonts.bold
              ], range: globalRange)
  ...
  private func paragraphStyle(from storage: MarkdownTextStorage, at location: Int)
    -> NSMutableParagraphStyle
  {
    if let textStorage = storage as? NSTextStorage,
      let existing = textStorage.attribute(.paragraphStyle, at: location, effectiveRange: nil)
        as? NSParagraphStyle
    {
  ...
  private func listParagraphStyle(mergingWith existing: NSMutableParagraphStyle)
    -> NSMutableParagraphStyle
  {
  ...
  private func quoteParagraphStyle(mergingWith existing: NSMutableParagraphStyle)
    -> NSMutableParagraphStyle
  {
    let indent = fonts.body.pointSize * 1.1
    existing.headIndent = max(existing.headIndent, indent)
    existing.paragraphSpacing = max(existing.paragraphSpacing, fonts.body.pointSize * 0.25)
    existing.paragraphSpacingBefore = max(
      existing.paragraphSpacingBefore, fonts.body.pointSize * 0.15)
    existing.tabStops = []
    existing.lineBreakMode = .byWordWrapping
    return existing
  }
...
private let strongAsteriskRegex = try! NSRegularExpression(
  pattern: "\\*\\*(.+?)\\*\\*", options: [])
private let strongUnderscoreRegex = try! NSRegularExpression(pattern: "__(.+?)__", options: [])
private let italicAsteriskRegex = try! NSRegularExpression(
  pattern: "(?<!\\*)\\*(?!\\*)(.+?)(?<!\\*)\\*(?!\\*)", options: [])
private let italicUnderscoreRegex = try! NSRegularExpression(
...
private let orderedListRegex = try! NSRegularExpression(
  pattern: "(?m)^(\\s*)(\\d+\\.)\\s+(.+)$", options: [])
private let blockquoteRegex = try! NSRegularExpression(
  pattern: "(?m)^(\\s{0,3}>\\s?)(.+)$", options: [])
private let linkRegex = try! NSRegularExpression(pattern: "\\[(.+?)\\]\\((.+?)\\)", options: [])
private let tableSeparatorRegex = try! NSRegularExpression(
  pattern: "^\\|?\\s*:?-{3,}:?\\s*(\\|\\s*:?-{3,}:?\\s*)+\\|?\\s*$", options: [])
...
extension MarkdownTextStorage {
  fileprivate func enumerateMatches(
    for regex: NSRegularExpression, contentGroup: Int, leadingTokenLength: Int,
    trailingTokenLength: Int, handler: (MarkdownRegexMatch) -> Void
  ) {
    ...
    for match in matches {
      ...
      let trailingTokenRange = NSRange(
        location: fullRange.upperBound - trailingTokenLength, length: trailingTokenLength)
      guard leadingTokenRange.isValid(within: length), trailingTokenRange.isValid(within: length),
        contentRange.isValid(within: length)
      else { continue }
      handler(
        MarkdownRegexMatch(
          fullRange: fullRange,
...
extension NSRange {
  fileprivate var upperBound: Int { location + length }

  fileprivate func isValid(within totalLength: Int) -> Bool {
    location >= 0 && upperBound <= totalLength && length >= 0
  }
}

extension NSTextCheckingResult {
  fileprivate func optionalRange(at index: Int) -> NSRange? {
  ...
  static var markdownCodeBackground: PlatformColor {
      ...
      UIColor { traits in
        traits.userInterfaceStyle == .dark
          ? UIColor.systemGray5 : UIColor.systemGray5.withAlphaComponent(0.6)
      }
    #else
